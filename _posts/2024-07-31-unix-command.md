...
- **shell** : 커맨드들을 컴퓨터가 이해할 수 있는 형태로 바꿔주는 역할   
    - bash, Ubuntu, zsh 등   
...
- **아큐먼트(argument)** : 어느 대상에 대해 커맨드를 실행할지 정함. 인자 라고도 함   
    - `cal`커맨드(달려출력)에 `2024`년 `5월`달력을 실행하기위한 아규먼트 설정   
...
## 디렉토리와 파일 다루기  
---
> 파일과 디렉토리를 다루는 커맨드를 알아보겠습니다.   
> 유닉스 운영체제에서는 폴더를 디렉토리로 칭합니다.   
...
- **파일 경로** : 루트에서부터 특정 파일이나 디렉토리까지의 경로를 사용할 때 디렉토리 안으로 들어갈 때는 `/`를 사용하고 루트는 아무 문자 없이 `/`만 사용   
    - root디렉토리 : `/`   
...
- 절대경로 : **루트 디렉토리** 기준 경로   
    - 여러번 하위 디렉토리로 이동하는 등 경로가 길어질수록 불편함 → 상대경로 고려   
      - 예시) summer디렉토리 이동 : `cd /home/Sun/market/summer`   
- 상대경로 : **현재 자신이 위치해 있는 디렉토리**(작업 디렉토리) 기준 경로   
    - 현재 디렉토리를 `.`으로 표시        
      - 예시) 현재 디렉토리 `/home/Sun/market`에서 summer디렉토리 이동 : `cd ./summer`   
      - 현재 디렉토리를 표시하는 `./` 생략 가능 `cd ./summer` = `cd summer`   
    - 현재 디렉토리에서 상위 디렉토리를 `..`으로 표시   
      - 예시) 현재 디렉토리 `/home/Sun/market/summer`에서 상위 디렉토리 Sun으로 이동 : `cd ../..`   
    ...
    - 여러번 상위 디렉토리로 이동해야하는 경우 → 절대경로 또는 홈 디렉토리 고려   
      - 예시) winter 디렉토리에서 animal 디렉토리로 이동   
        ...
        - 홈 디렉토리 사용 : `cd ~/animal`
- 경로를 아규먼트로 받는 모든 커맨드에 절대경로, 상대경로 사용 가능   
...
> 별도 확인 없이 파일을 덮어씀   
> 파일을 덮어쓰기전에 확인하는 과정을 만들고 싶다면 `-i`옵션을 사용     
{: .prompt-warning }   
